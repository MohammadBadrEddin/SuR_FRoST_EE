...
\begin{figure} [h]
    \centering
    \includegraphics[width=0.8\linewidth]{Abbildung/Programmablauf.png}
...
\begin{itemize}
    ...
    \item \texttt{TMC2209\_SetStallguardThreshold()} und \texttt{TMC2209\_SetCoolStepThreshold()} konfigurieren StallGuard4 (\texttt{SGTHRS}) bzw. dessen Gültigkeitsfenster (\texttt{TCOOLTHRS}). Beide Werte bilden die Grundlage für die im \texttt{ControlTask} realisierte StallGuard-basierte Kraftregelung.
    \item \texttt{TMC2209\_MoveVelocity()} schreibt eine Solldrehzahl in das \texttt{VACTUAL}-Register (24 Bit, vorzeichenbehaftet). In diesem Betriebsmodus erzeugt der TMC2209 die Mikroschritt-Pulse \emph{intern}; der Mikrocontroller muss keine STEP-Flanken mehr ausgeben. \texttt{TMC2209\_Stop()} setzt \texttt{VACTUAL} auf 0.
\end{itemize}

\subsection{Motor-Control-Schicht}

\texttt{motor\_control.c} bündelt die Inbetriebnahme des Treibers und stellt \emph{zwei alternative} Mechanismen zur Bewegungserzeugung bereit, die im Code parallel existieren, aber unterschiedlich stark genutzt werden.

Die Datei\texttt{MotorControl\_Init()} ruft \texttt{TMC2209\_Init()} in einer Retry-Schleife auf (Wiederholung alle 500\,ms bis zur erfolgreichen Antwort, statt eines harten Abbruchs), setzt anschließend feste Betriebsparameter (16 Mikroschritte, \texttt{IRUN=IHOLD=27} für ca.\ 0{,}86\,A\textsubscript{RMS} bei $R_\text{sense}=0{,}11\,\Omega$, ein konservatives StallGuard/CoolStep-Fenster) und aktiviert den Treiber über den Enable-Pin. Über \texttt{MotorControl\_GetDriver()} erhalten andere Module (insbesondere der \texttt{ControlTask}) Zugriff auf den initialisierten TMC2209-Handle, um darüber \texttt{TMC2209\_MoveVelocity()} bzw.\ \texttt{TMC2209\_Read()} (StallGuard) direkt aufzurufen. \textbf{Dieser UART/VACTUAL-Pfad ist der einzige Bewegungsmechanismus, der im laufenden System tatsächlich verwendet wird}, da der geschlossene Regelkreis im \texttt{ControlTask} ausschließlich über \texttt{TMC2209\_MoveVelocity()} kommandiert wird.

Daneben implementiert das Modul einen zweiten, STEP/DIR-basierten Bewegungsmechanismus: \texttt{MotorControl\_Move()} berechnet mit dem Austin/Aryeh-Rampenalgorithmus \cite{austin2005} eine ganzzahlige Trapezrampe (Beschleunigen – Reisegeschwindigkeit – Bremsen) und startet einen Hardware-Timer (TIM3) im Interrupt-Betrieb. \texttt{MotorControl\_TimerISR()}, die aus \texttt{HAL\_TIM\_PeriodElapsedCallback()} aufgerufen wird, toggelt bei jedem Timer-Überlauf den STEP-Pin, passt den Autoreload-Wert gemäß der Austin-Iteration an und aktualisiert einen Positionszähler (\texttt{s\_positionFullSteps}). \texttt{MotorControl\_WaitIdle()} blockiert den aufrufenden Task per Task-Notification bis zum Bewegungsende. Dieser Mechanismus wird jedoch ausschließlich von \texttt{MotorInitAndTestTask} verwendet – einer Funktion, die im aktuellen Build \emph{nicht} über \texttt{xTaskCreate()} gestartet wird (vgl. Task-Liste in Abschnitt~\ref{fig:programmablauf}). Er ist damit als vorbereitete, aber im produktiven Regelbetrieb ungenutzte Infrastruktur für offene Schrittkommandos zu verstehen.

\paragraph{Anmerkung zur Positionsermittlung.} Da die geschlossene Regelung ausschließlich über \texttt{VACTUAL} erfolgt und der TMC2209 die Mikroschritt-Pulse intern erzeugt, wird \texttt{s\_positionFullSteps} – und damit \texttt{MotorControl\_GetPositionMicrosteps()} – im produktiven Betrieb nicht fortgeschrieben, da \texttt{MotorControl\_TimerISR()} nur bei aktiver STEP/DIR-Bewegung inkrementiert. Der im \texttt{ControlTask} geloggte Schrittzähler (\texttt{step\_cnt}) wird daher unabhängig davon als softwareseitige Integration der kommandierten Geschwindigkeit (\texttt{vactual}) über die Zykluszeit berechnet und stellt somit eine rein rechnerische Schätzung der kommandierten Bewegung dar, keine hardwareseitig gezählte Pulszahl. Die tatsächliche Positionsrückführung für die Regelung erfolgt ausschließlich über den externen AS5600-Encoder.
